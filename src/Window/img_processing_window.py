...
class IMGProcessingWindow :
    ...
    def __init__(self, root ,img):
        ...
        # ウィンドウの作成
        self.img_processing_window = Toplevel()
        self.img_processing_window.title("検査画像編集")
        self.img_processing_window.geometry("900x470")  # Set window size (width x height)
        self.img_processing_window.resizable(False, False)        # サイズを固定

        self.frame = tk.Frame(self.img_processing_window, height = self.Canvas_size[0])
        ...
        self.save_button    = tk.Button(self.frame, text="画像を保存",  command=self.SeaveImageAnalusis)  
        self.cancel_button  = tk.Button(self.frame, text="更新せず閉じる" , command=self.CancelProcessing)  
        self.clause_button  = tk.Button(self.frame, text="更新して閉じる" , command=self.CloseWindow )
        ...
        self.clause_button.grid(row=4, column=1, pady=20)
         

    
        
        self.canvas = tk.Canvas( self.img_processing_window, width = self.Canvas_size[0], height = self.Canvas_size[1], bg="gray",highlightthickness=1, highlightbackground="black", highlightcolor="black" )
        ...
        self.img_processing_window.wait_window()
    ...
    def SeaveImageAnalusis(self):
        ...
        try:
            ...
            result, n = cv2.imencode(ext, self.copy_img   , None)
            self.isupdate  = True
            if result:
    ...
    def ReturnImg(self):
        return self.isupdate ,self.copy_img
